        ...
        if(request.axis == 1):
            if request.idle == 1:
                ...
                if request.giro == 1:
                    ...
                    odrv0.axis1.controller.input_vel = -0.5 if request.ang >= 0 else 0.5
            ...
            if request.idle == 1:
                ...
                if request.giro == 1:
                    ...
                    beta = 1.523 
                    D = 0.1651
                    V = (2*math.pi*H*(request.ang - beta)/(360*math.pi*D)) if (request.ang >= 0) else ((2*math.pi*H*(request.ang + beta)/(360*math.pi*D)))
                    odrv0.axis1.controller.config.control_mode = CONTROL_MODE_VELOCITY_CONTROL
                    odrv0.axis0.controller.config.control_mode = CONTROL_MODE_VELOCITY_CONTROL
                    odrv0.axis1.requested_state = AXIS_STATE_CLOSED_LOOP_CONTROL
                    odrv0.axis0.requested_state = AXIS_STATE_CLOSED_LOOP_CONTROL
                    ...
                    print("Cuenta = ", V)
                    odrv0.axis0.controller.input_vel = 0.5 if request.ang >= 0 else -0.5
                    odrv0.axis1.controller.input_vel = 0
                    print("Inicio espera")
